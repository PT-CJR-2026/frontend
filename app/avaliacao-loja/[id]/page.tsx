...
export default function AvaliacaoLojaPage() {
  ...
  // ATENÇÃO: confirme com o backend se a rota de comentários de loja
  // é /comentario-avaliacao/loja/:avaliacaoLojaId (já existe no service)
  async function carregarComentarios() {
    try {
      const response = await axiosInstance.get(`/comentario-avaliacao/loja/${id}`);
  ...
  useEffect(() => {
    async function carregar() {
      try {
        const avaliacaoResponse = await axiosInstance.get(`/avaliacao-loja/${id}`);
        setAvaliacao(avaliacaoResponse.data);

        // Busca o nome da loja separadamente, já que a avaliação só traz loja_id
        const lojaId = avaliacaoResponse.data.loja_id;
        if (lojaId) {
          try {
            const lojaResponse = await axiosInstance.get(`/lojas/${lojaId}`);
            setNomeLoja(lojaResponse.data.nome ?? "Loja");
          } catch (err) {
            console.log("Erro ao carregar nome da loja:", err);
          }
        }
      } catch (err: any) {
